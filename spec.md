...
> Documentación del estado actual del código. Última revisión: 2026-07-30.
...
### `Comment` — tabla `comentarios`

> La clase se renombró a `Comment` por convención de nombres en inglés. El nombre físico de la tabla (`comentarios`) y la columna (`comentario` → mapeada con `@Column(name = "comentario")`) se conservan para no alterar la DB.

| Campo      | Tipo           | Restricciones / Defaults                              |
|------------|----------------|-------------------------------------------------------|
| `id`       | Long           | PK, auto-increment                                   |
| `createdAt`| LocalDateTime  | Default: `LocalDateTime.now()` al instanciar la entidad |
| `comment`  | String         | Mapea a columna `comentario`. Requerido, no nulo, no vacío |
...
**Request body (`SaveCommentDto`):**
```json
{ "comment": "Revisé el equipo, falta el repuesto X." }
```

**Respuesta 201:** `CommentDto` en el body + header `Location: /ticket/{ticketId}/comments/{id}`.

| Código | Caso |
|--------|------|
| 201    | Comentario creado |
| 400    | `comment` nulo o vacío, o JSON malformado |
| 401    | Sin JWT válido |
| 404    | No existe ticket con ese `ticketId` |
...
| `GET /ticket/{id}/comments`     | JWT válido (cualquier rol) |
| `POST /ticket/{id}/comments`    | JWT válido (cualquier rol) |
...
├── dtos/            CommentDto, ErrorDto, SaveCommentDto, SaveTicketDto, TicketDto,
...
└── services/        CommentService/Impl, TicketService/Impl, UserService/Impl, UserDetailsServiceImpl
```
